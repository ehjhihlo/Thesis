...
def get_person_detection_boxes(model, img, threshold=0.5):
    ...
    for idx, box in enumerate(pred_boxes):
        if person_boxes == [] and pred_classes[idx] == 'person':
...
def get_pose_estimation_prediction(pose_model, image, center, scale):
    ...
    model_input = transform(model_input).unsqueeze(0).cuda()
...
def parse_args():
    ...
    parser.add_argument('--cfg', type=str, default='./experiments/w48_384x288_adam_lr1e-3.yaml')
...
def main():
    ...
    print('=> loading model ...')
    pose_model.load_state_dict(torch.load('../checkpoint/pose_hrnet_w48_384x288.pth'), strict=False)
    ...
    if args.webcam or args.video:
            ...
            if ret:
                ...
                if len(pred_boxes) >= 1:
                    for box in pred_boxes:
                        ...
                        pose_preds, maxvals = get_pose_estimation_prediction(pose_model, image_pose, center, scale)
                        # get_pose_estimation_prediction(pose_model, image_pose, center, scale)
                        keypoints, scores, valid_frames = h36m_coco_format(pose_preds, maxvals)
                        # if len(pose_preds)>=1:
                        #     for kpt in pose_preds:
                        #         draw_pose(kpt,image_bgr) # draw the poses
                        show2Dpose(keypoints[0][0], image_bgr)
        ...
        if len(pred_boxes) >= 1:
            for box in pred_boxes:
                ...
                pose_preds, scores, valid_frames = h36m_coco_format(pose_preds, scores)
                if len(pose_preds)>=1:
                    for kpt in pose_preds:
                        draw_pose(kpt,image_bgr) # draw the poses
