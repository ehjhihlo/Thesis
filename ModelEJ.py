...
class GLBlockEJ(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.att_temporal(self.graph_spatial(x))
...
def create_layers(dim, n_layers, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop_rate=0., drop_path_rate=0.,
    ...
    for _ in range(n_layers):
        layers.append(GLBlockEJ(dim=dim,
                                mlp_ratio=mlp_ratio,
                                act_layer=act_layer,
                                attn_drop=attn_drop,
                                drop=drop_rate,
...
class OcclusionAttentionBlock(nn.Module):
    def __init__(self, num_heads, in_channels, dim_feat, n_joints, out_channels):
        super().__init__()

        self.num_heads = num_heads
        self.joints_embed = nn.Linear(in_channels, dim_feat)
        self.pos_embed = nn.Parameter(torch.zeros(1, n_joints, dim_feat))
        
        self.v_embed = nn.Linear(in_channels, dim_feat)
        self.acc_embed = nn.Linear(in_channels, dim_feat)

        self.scale = dim_feat ** -0.5

        # self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim_feat, dim_feat)
        # self.qkv = nn.Linear(args.in_channels, args.in_channels * 3, bias=qkv_bias)
    ...
    def forward(self, x):
        ...
        zero_list = np.zeros((B, 1, J, C))
        v = np.concatenate((zero_list, v), axis=1)
        a = v[:, 1:] - v[:, :-1]
        a = np.concatenate((zero_list, a), axis=1)
        ...
        v = self.v_embed(v.float())
        a = self.acc_embed(a.float())
...
class MyModel(nn.Module):
    ...
    #              neighbour_num=4, n_frames=243, channel=256, stride_num=[3,9,9]):
    def __init__(self, args):
        ...
        self.fcn = nn.Sequential(
            ...
            nn.Conv1d(args.dim_feat, 3*args.n_joints, kernel_size=1)
        )

        self.occlusion = OcclusionAttentionBlock(args.num_heads, args.in_channels, args.dim_feat, args.n_joints, args.out_channels)
        ...
        self.head = nn.Linear(args.dim_rep, args.out_channels)
