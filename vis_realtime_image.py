...
def generate_3D_pose(args, opts, keypoints_cache, image_2d, output_dir):
    ...
    if opts.usebaseline:
        ...
        plt.savefig(os.path.join(output_dir, f'pose3d_{opts.image_path}_baseline.png'))
    else:
        # plt.savefig(os.path.join(output_dir, f'mpi-pose3d_{opts.image_path}.png'))
        plt.savefig(os.path.join(output_dir, f'pose3d_{opts.image_path}.png'))
    plt.close(fig)
...
def parse_args():
    ...
    parser.add_argument('--image-path', type=str, default='S9_Discussion_1.58860488_001151.jpg')
...
if __name__ == "__main__":
    ...
    if opts.camera == 'xtion_pro':
        ...
        frame = cv2.imread(os.path.join(opts.root, opts.image_path))
        cv2.imwrite(os.path.join(output_dir, f'org_{opts.image_path}.png'), frame)
    ...
    cv2.imwrite(os.path.join(output_dir, f'pose2d_{opts.image_path}.png'), image_2d)
    ...
    generate_3D_pose(args, opts, keypoints_cache_r, image_2d, output_dir)
